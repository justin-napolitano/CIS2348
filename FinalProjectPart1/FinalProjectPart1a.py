...
file = [*csv.DictReader(open("ManufacturerList.csv"),fieldnames = ("item_id","Man_name","Type","Condition","Price", "Date"))]
...
#item_id is used to merge
        c[combined['item_id']].update(combined)
#adds the values of the 'c' dictionary to PriceAndServiceDates
PriceAndServiceDates = c.values()

d = defaultdict(dict)
#combines ManufacturerList and PriceList
for l in (ManufacturerList, PriceList):
    for elem in l:
       d[elem['item_id']].update(elem)

ManufacturerAndPrice = d.values()
